...
class PatchEmbed(nn.Module):
    def __init__(self, image_size = [32, 256], in_channels = 3, embed_dim = 768, max_length = 16):
        super(PatchEmbed, self).__init__()
        
        self.image_size = image_size
        self.patch_size = [image_size[0], int(image_size[1]/max_length)]
        self.n_patches = max_length
        self.proj = nn.Conv2d(in_channels, embed_dim, kernel_size=self.patch_size, stride=self.patch_size[1])
...
class VisionTransformer(nn.Module):
    def __init__(self, image_size = [32, 256], in_channels = 3, embed_dim = 384, depth = 12, n_heads = 12, qvk_bias = True, attn_p=0., proj_p=0., mlp_hidden_ratio = 4, mlp_p = 0.1, p = 0.):
        super().__init__()
        self.patch_embed = PatchEmbed(image_size=image_size, in_channels= in_channels, embed_dim = embed_dim)
